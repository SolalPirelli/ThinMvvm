...
namespace ThinMvvm
{
    ...
    {
        private readonly Dictionary<string, ICommand> _commandCache = new Dictionary<string, ICommand>();
        ...
        /// <summary>
        /// Gets or creates a <see cref="Command" /> that will be unique to the <see cref="ViewModel{TParameter}" />.
        /// </summary>
        ...
        protected Command GetCommand( Action execute, Expression<Func<bool>> canExecute = null, [CallerMemberName] string name = "" )
        {
            ...
            {
                _commandCache.Add( name, new Command( this, execute, canExecute ) );
            }

            return (Command) _commandCache[name];
        }

        /// <summary>
        /// Gets or creates a <see cref="Command{T}" /> that will be unique to the <see cref="ViewModel{TParameter}" />.
        /// </summary>
        ...
        protected Command<T> GetCommand<T>( Action<T> execute, Expression<Func<T, bool>> canExecute = null, [CallerMemberName] string name = "" )
        {
            if ( !_commandCache.ContainsKey( name ) )
            {
                _commandCache.Add( name, new Command<T>( this, execute, canExecute ) );
            }

            return (Command<T>) _commandCache[name];
        }
        ...
        /// <summary>
        /// Gets or creates an <see cref="AsyncCommand" /> that will be unique to the <see cref="ViewModel{TParameter}" />.
        /// </summary>
        /// <param name="execute">The action to execute when the command is executed.</param>
        /// <param name="canExecute">Optional. The predicate indicating whether the command can be executed.</param>
        /// <param name="name">Optional. Should not be provided as it uses compiler functionality. The command name.</param>
        protected AsyncCommand GetAsyncCommand( Func<Task> execute, Expression<Func<bool>> canExecute = null, [CallerMemberName] string name = "" )
        {
            if ( !_commandCache.ContainsKey( name ) )
            {
                _commandCache.Add( name, new AsyncCommand( this, execute, canExecute ) );
            }

            return (AsyncCommand) _commandCache[name];
        }

        /// <summary>
        /// Gets or creates an <see cref="AsyncCommand{T}" /> that will be unique to the <see cref="ViewModel{TParameter}" />.
        /// </summary>
        /// <typeparam name="T">The command's argument type.</typeparam>
        /// <param name="execute">The action to execute when the command is executed.</param>
        /// <param name="canExecute">Optional. The predicate indicating whether the command can be executed.</param>
        ...
        protected AsyncCommand<T> GetAsyncCommand<T>( Func<T, Task> execute, Expression<Func<T, bool>> canExecute = null, [CallerMemberName] string name = "" )
        {
            if ( !_commandCache.ContainsKey( name ) )
            {
                _commandCache.Add( name, new AsyncCommand<T>( this, execute, canExecute ) );
            }

            return (AsyncCommand<T>) _commandCache[name];
        }
    }
}